const GLM_API_URL = process.env.VISION_API_URL || "https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions";
const GLM_MODEL = process.env.VISION_MODEL || "qwen3.7-flash";
const REQUEST_TIMEOUT_MS = 60_000;
/** 免费模型容易被限流（429），按间隔重试几次 */
...
export interface DescribeImageParams {
  ...
  thinking: boolean;
  language: Language;
  /** 裁剪后的区域标签，如 "top-right (100,100,300,200px)"；仅当裁剪过时非空 */
  regionLabel?: string;
}
...
function buildSystemPrompt(language: Language): string {
  ...
  return [
    "你是 Qwen 视觉模型（qwen3.7-flash），通过 MCP 工具为 Codex 提供识图能力。",
...
export async function describeImage(params: DescribeImageParams): Promise<string> {
  ...
  for (let attempt = 0; attempt <= RETRY_DELAYS_MS.length; attempt++) {
    ...
    try {
      ...
      const networkError = new Error(`调用视觉模型 API 失败（网络或超时）：${message}`);
    ...
    if (response.ok) {
      ...
      try {
        ...
        throw new Error(`视觉模型 API 返回了无法解析的响应：${raw.slice(0, 300)}`);
      }
...
function formatApiError(status: number, raw: string): string {
  ...
  if (status === 401) {
    return `API Key 无效或已过期（401）：请检查 Codex 配置 [mcp_servers.vision.env] 中的 QIANWEN_API_KEY。详情：${detail}`;
  }
  if (status === 429) {
  ...
  return `视觉模型 API 返回错误（${status}）：${detail}`;
}
